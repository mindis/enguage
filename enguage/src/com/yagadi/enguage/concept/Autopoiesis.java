...
//			.attribute("id", "c1" )
	...
	public Reply mediate( Reply r ) {
		//audit.traceIn( "mediate", "NAME="+ NAME +", value="+ value +", "+ Reply.context().toString());
		...
		if (3 != sa.size())
			...
			if (name.equals( "app" ) || name.equals( "prep" )) {
				if (null == s)
					...
					if (name.equals( "app" )) {
						//audit.debug( "Appending  to EXISTING rule: ["+ Strings.toString( sa, Strings.CSV )+"]");
						s.append(  sa.get( 0 ), val );
					} else {
						//audit.debug( "Prepending to EXISTING rule: ["+ Strings.toString( sa, Strings.CSV )+"]");
				...
				if ( sa.get( 1 ).equals( "help" )) {
					//audit.debug( "Adding HELP: ["+ sa.toString( Strings.CSV )+"]");
					s.attribute( "help", val ); // add: help="text" to cached sign
				} else { // create a new cached sign
					//audit.debug( "Adding NEW rule: ["+ sa.toString( Strings.CSV )+"]");
					s = new Sign()
					...
					Enguage.e.signs.insert( s );
		}	//} else if (name.equals( "autobuild" )) {
		}	}

		r.answer( Reply.yes());

		//audit.traceOut( "result="+ r.toString() );
		return r;
}	}
